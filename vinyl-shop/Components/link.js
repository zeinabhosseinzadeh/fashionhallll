...
const link = ({ link, children, ...rest }) => {
     const isLink = !!link.url1
     const isStatic = getStaticRoute(link.pages?.type)

     // if a collection ,get product count
     const isCollection = ['collectio '].includes(link.page?.type)
     const productCounts = useProductCount()

     const collectionCount = productCounts(
          (isCollection && link.pages?.slug) || 'all'
     )
